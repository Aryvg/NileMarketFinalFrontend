...
async function fetchUsersFromBackend() {
  try {
    ...
    const res = await fetch('https://nilemarketfinalbackend.onrender.com/users', { headers });
...
async function checkDashboardRole() {
  try {
    const res = await fetch('https://nilemarketfinalbackend.onrender.com/refresh/status', {
      method: 'GET',
      credentials: 'include'
    });
    if (res.ok) {
...
(function initCounts() {
  ...
  if (typeof productCount !== 'undefined') {
    // Show 0 if no products, not storedProd (which could be stale/7)
...
async function fetchProductsFromBackend() {
  try {
    const token = localStorage.getItem('accessToken');
    const headers = token ? { 'Authorization': 'Bearer ' + token } : {};
    const res = await fetch('https://nilemarketfinalbackend.onrender.com/products?showHidden=1', { headers });
    if (!res.ok) throw new Error('Failed to fetch products');
    const data = await res.json();
    // Store products with _id and hidden property
...
async function fetchOrdersAndUsers() {
  try {
    ...
    const token = localStorage.getItem('accessToken');
    const headers = token ? { 'Authorization': 'Bearer ' + token } : {};
    const usersRes = await fetch('https://nilemarketfinalbackend.onrender.com/users', { headers });
    let usersData = [];
    if (usersRes.ok) {
    ...
    const ordersRes = await fetch('https://nilemarketfinalbackend.onrender.com/orders?all=1', { headers });
    let ordersData = [];
    if (ordersRes.ok) {
...
`;
  ...
  row.querySelector(".deleteBtn").onclick=()=>{
    ...
    document.getElementById('confirmDeleteYes').onclick = async function() {
      ...
      try {
        ...
        const res = await fetch(`https://nilemarketfinalbackend.onrender.com/users/${u._id}`, {
          method: 'DELETE',
          headers
        });
  ...
  roleSel.onchange = async e => {
    ...
    async function ensureAccessToken() {
      ...
      try {
        const r = await fetch('https://nilemarketfinalbackend.onrender.com/refresh', { method: 'GET', credentials: 'include' });
    ...
    if (u._id) {
      try {
        ...
        const res = await fetch(`https://nilemarketfinalbackend.onrender.com/users/${u._id}/role`, {
          method: 'PUT',
          headers,
...
addProductBtn.onclick = async () => {
  ...
  try {
    ...
    const uploadRes = await fetch('https://nilemarketfinalbackend.onrender.com/media/upload', {
      method: 'POST',
      body: imageForm,
    ...
    if (!uploadRes.ok) {
      ...
      if (uploadRes.status === 409) {
        // Show professional styled popup for duplicate
        let popup = document.getElementById('duplicate-image-popup');
        if (!popup) {
          popup = document.createElement('div');
          popup.id = 'duplicate-image-popup';
          popup.style.position = 'fixed';
          popup.style.top = '50%';
          popup.style.left = '50%';
          ...
          popup.innerHTML = `
            <div style="font-weight:600;font-size:22px;margin-bottom:12px;color:#e74a3b;">Duplicate Image Detected</div>
            <div style="margin-bottom:18px;">${msg}</div>
            <button id="close-duplicate-popup" style="margin-top:8px;padding:10px 28px;background:#0077ff;color:white;border:none;border-radius:8px;font-size:17px;cursor:pointer;">OK</button>
          `;
          document.body.appendChild(popup);
          document.getElementById('close-duplicate-popup').onclick = function() {
            popup.remove();
          };
        }
    ...
    // 2. Save product with Cloudinary image URL
    const formData = new FormData();
    formData.append('image', imageUrl);
    formData.append('name', name);
    formData.append('priceCents', priceCents);
    const res = await fetch('https://nilemarketfinalbackend.onrender.com/products?showHidden=1', {
      method: 'POST',
      body: formData,
      headers: accessToken ? { 'Authorization': 'Bearer ' + accessToken } : {}
    });
...
products.filter(p => p.name.toLowerCase().includes(filter.toLowerCase())).slice().reverse()
  ...
  .forEach((p) => {
    ...
    if (imgSrc && !/^https?:\/\//.test(imgSrc)) {
      imgSrc = `https://nilemarketfinalbackend.onrender.com/${imgSrc}`;
    }

    // Hide/Unhide button markup
    const isHidden = !!p.hidden;
    const hideBtnStyle = isHidden
      ? 'background:#f39c12;color:white;border:none;margin-left:8px;font-size:15px;padding:6px 16px;'
      : 'background:#6c757d;color:white;border:none;margin-left:8px;font-size:15px;padding:6px 16px;';
    const hideBtnText = isHidden ? 'Unhide' : 'Hide';

    // Add a badge and faded style if hidden
    const hiddenBadge = isHidden ? '<span class="hidden-badge" style="position:absolute;top:8px;right:8px;background:#f39c12;color:white;padding:2px 10px;border-radius:12px;font-size:13px;z-index:2;">Hidden</span>' : '';
    const boxOpacity = isHidden ? '0.6' : '1';
    box.innerHTML = `
      ...
      <button class="deleteBtn">Delete</button>
      <button class="hideBtn" style="${hideBtnStyle}">${hideBtnText}</button>
    `;
    ...
    hideBtn.onclick = async () => {
      ...
      if (!productId) return alert('Product missing ID.');
      hideBtn.disabled = true;
      const originalText = hideBtn.textContent;
      hideBtn.textContent = '...';
      try {
        const res = await fetch(`https://nilemarketfinalbackend.onrender.com/products/toggle-hidden/${productId}`,
          {
            method: 'PATCH',
            headers: {
        ...
        // Re-render the products list to update button, badge, and style
        renderProducts(productSearch.value);
      } catch (err) {
        alert('Error toggling hidden: ' + err.message);
      } finally {
        hideBtn.disabled = false;
        if (!hideBtn.textContent) hideBtn.textContent = originalText;
      }
    };
    deleteBtn.onclick = () => {
      const accessToken = localStorage.getItem('accessToken');
      ...
      deleteBtn.innerHTML = '<span class="btn-spinner" style="display:inline-block;width:18px;height:18px;border:3px solid #fff;border-top:3px solid #0077ff;border-radius:50%;animation:spinBtn 0.8s linear infinite;vertical-align:middle;"></span>';
      // Add spinner keyframes if not present
      if (!document.getElementById('btn-spinner-style')) {
        const style = document.createElement('style');
        style.id = 'btn-spinner-style';
        style.innerHTML = `@keyframes spinBtn{0%{transform:rotate(0deg);}100%{transform:rotate(360deg);}}`;
        document.head.appendChild(style);
      }
      if (productId) {
        fetch('https://nilemarketfinalbackend.onrender.com/products?showHidden=1', {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
            ...(accessToken ? { 'Authorization': 'Bearer ' + accessToken } : {})
