...
export async function registerHandler(fields, validateField) {
          ...
          try {
            const r = await fetch(`https://nilemarketfinalbackend.onrender.com/users/exists?user=${encodeURIComponent(username)}`);
            if (r.ok) {
          ...
          try {
            const res = await fetch('https://nilemarketfinalbackend.onrender.com/register', {
              method: 'POST',
            ...
            if (!res.ok) {
              ...
              try {
                const authRes = await fetch('https://nilemarketfinalbackend.onrender.com/auth', {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  credentials: 'include',
