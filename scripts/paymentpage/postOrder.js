export async function placeOrder(event) {
  ...
  try {
    ...
    // 1️⃣ GET CART DATA
    const cartResponse = await fetch('https://nilemarketfinalbackend.onrender.com/cart', {
      headers: {
    ...
    const orderResponse = await fetch('https://nilemarketfinalbackend.onrender.com/orders', {
      method: 'POST',
      headers: {
