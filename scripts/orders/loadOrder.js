...
import { isProductHidden } from '../utils/isProductHidden.js';
...
export async function loadOrder(searchTerm) {
  try {
    ...
    let url = 'https://nilemarketfinalbackend.onrender.com/orders';
    ...
    if (out) {
      ...
      out.querySelectorAll('.js-buy-again-button').forEach(btn => {
        btn.addEventListener('click', async () => {
          ...
          const unavailable = await isProductHidden(name);
          if (unavailable) {
            // Show popup for unavailable product
            let popup = document.getElementById('unavailable-product-popup');
            if (!popup) {
              popup = document.createElement('div');
              popup.id = 'unavailable-product-popup';
              ...
              popup.innerHTML = 'This product is currently unavailable.' +
                '<br><br><button id="close-unavailable-product-popup" style="margin-top:18px;padding:8px 22px;background:#0077ff;color:white;border:none;border-radius:8px;font-size:16px;cursor:pointer;">OK</button>';
              document.body.appendChild(popup);
              document.getElementById('close-unavailable-product-popup').onclick = function() {
                popup.remove();
              };
            }
            return;
